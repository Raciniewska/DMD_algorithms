...
class rDMDOperator(object):
    ...
    def compute_operator(self, X, Y,Q):
        """
        ...
        self._Atilde = atilde
        self._compute_eigenquantities() #self._eigenvalues, self._eigenvectors
        self._compute_modes(Y, U, s, V, Q) #Tu zmiana bo randomizacja dodana Q
    ...
    def _compute_modes(self, Y, U, Sigma, V, Q):
        """
        Private method that computes eigenvalues and eigenvectors of the
        ...
        if self._exact:
            print("mapping modes in operator - Exact")
            high_dimensional_eigenvectors = ((Q.dot(Y.dot(V) *
                             np.reciprocal(Sigma)).dot(W)))


        else:
            print("mapping modes in operator")
            high_dimensional_eigenvectors = Q*U.dot(W)
