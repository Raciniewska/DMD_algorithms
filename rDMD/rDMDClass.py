...
from rDMD.rDMDBase import rDMDBase
...
class rDMDClass(rDMDBase):
    ...
    def fit(self, X,oversample,n_subspace,random_state):
        """
        ...
        X = self._snapshots[:, :-1]
        Y = self._snapshots[:, 1:]
        print("oryg snapshots shape:")
        print( self._snapshots.shape)

        #noice reduction
        X, Y = compute_tlsq(X, Y, self.tlsq_rank)

        # Compute QB decomposition
        Q, B = compute_rqb(self._snapshots, self.svd_rank, oversample=oversample, n_subspace=n_subspace,
        ...
        B_y=B[:, 1:]
        print(len(B))
        print("Len of smaples")
        print(len(B[0]))

        self._svd_modes, _, _ = self.operator.compute_operator(B_x,B_y,Q)
        print("operator computed")
